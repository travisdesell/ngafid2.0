...
public class Event {
    ...
    public static HashMap<String, ArrayList<Event>> getEvents(Connection connection, int fleetId,
        ...
        System.out.println("========================================");
        System.out.println("FETCHING EVENTS: " + eventName);
        System.out.println("Fleet ID: " + fleetId + ", Date Range: " + startTime + " to " + endTime + ", Tag: " + tagName);
        System.out.println("========================================");
        ...
        if (Objects.equals(tagName, "All Tags")) {
            eventsQuery = "SELECT events.id, events.flight_id, events.start_line, events.end_line, events" +
                    ...
                    "flights.system_id, tails.tail, events.event_definition_id FROM events, flights, tails WHERE events.flight_id = flights" +
                    ".id AND flights.system_id = tails.system_id AND events.event_definition_id IN (";
        } else {
            eventsQuery = "SELECT events.id, events.flight_id, events.start_line, events.end_line, events" +
                    ".start_time, events.end_time, events.severity, events.other_flight_id, flights.airframe_id, " +
                    "flights.system_id, tails.tail, flight_tags.name, events.event_definition_id FROM events, flights, tails, flight_tag_map," +
                    ...
                    "flight_tag_map.tag_id = flight_tags.id AND events.event_definition_id IN (";
        }
        ...
        System.out.println("========================================");
        System.out.println("FETCH COMPLETE: " + eventName);
        System.out.println("Total events fetched: " + totalEvents);
