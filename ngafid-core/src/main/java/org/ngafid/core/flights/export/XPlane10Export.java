    ...
    public void writeFlightData(StringBuffer buffer, Map<String, Object> scopes) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (!Double.isNaN(parameters.get(LONGITUDE).get(i))
                ...
                buffer.append("DATA, " + i + "," + NULL_DATA
                        + parameters.get(LONGITUDE).get(i) + ","
                        + parameters.get(LATITUDE).get(i)
                        + "," + parameters.get(ALT).get(i) + "," + getZeros(4)
                        + parameters.get(PITCH).get(i) + ","
                        + parameters.get(ROLL).get(i) + ","
                        + parameters.get(HEADING).get(i)
                        + "," + parameters.get(IAS).get(i) + getZeros(70) + (Double.isNaN(e1EGT) ? "0" : e1EGT)
                        + getZeros(19) + "\n");
            }
        }
    }
