...
export function SeveritiesPage() {
    ...
    (selectedAirframe: string) => {
      ...
      severitiesPlot.on("plotly_click", (event) => {
        ...
        for (const point of event.points) {
          ...
          const flightIds = other ? [other, flightId] : [flightId];
  ...
  const dateChange = useCallback(async () => {
    ...
    for (const eventName of eventNames) cleared[eventName] = false;
