...
export function useCartesianZoomPan({
    ...
    const startZoomAnimation = (
        ...
        const step = () => {
            ...
            const t = Math.max(0.00, Math.min(1.00, tRaw));
            const eased = 1.00 - Math.pow(1.00 - t, 3.00); //<--p easeOutCubic
    ...
    const getChartScales = useCallback(() => {
        ...
        if (!chartAny || typeof chartAny.getXScales !== "function" || typeof chartAny.getYScales !== "function")
            return null;
        ...
        if (!xScale || !yScale || typeof xScale.invert !== "function" || typeof yScale.invert !== "function") {
            return null;
        }
    ...
    const getChartContainerRect = (): DOMRect | null => {
        ...
        // Container element not found, exit
        if (!container)
            return null;
        ...
        if (applyX && currentXDomain) {
            ...
            if (Number.isFinite(xCenter)) {
                ...
                if (Number.isFinite(currXSpan) && currXSpan > 0) {
                    ...
                    // Clamp into base domain
                    // [xMin, xMax] = clampDomain(xMin, xMax, baseXMin, baseXMax);
        ...
        if (applyY && currentYDomain) {
            ...
            if (Number.isFinite(yCenter)) {
                ...
                if (Number.isFinite(currYSpan) && currYSpan > 0) {
                    ...
                    // Clamp into base domain
                    // [yMin, yMax] = clampDomain(yMin, yMax, baseYMin, baseYMax);
    ...
    // Global pointermove: keeps pan & zoom selection updating even outside chart.
    useEffect(() => {
    ...
    // Global pointerup / pointercancel: apply zoom / finish pan even if released outside chart.
    useEffect(() => {
