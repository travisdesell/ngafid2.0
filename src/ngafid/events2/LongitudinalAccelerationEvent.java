...
public class LongitudinalAccelerationEvent {

    public static void calculateEvents(Connection connection) {
        try {
            // I set the Longitudinal Acceleration as GndSpd only for testing. The correct parameter should be decided and set in here. 
            String columnName = "GndSpd";
            // correct values for Longitudinal Acceleration is min:-1.76 or max=4.4; 
            double minValue = -1.76;
            double maxValue = 184.4;

            //grab the flight IDs that can have a Longitudinal Acceleration event
            ...
            while (flightsRS.next()) {
                //get the ID from each flight that had a Longitudinal Acceleration event
                int flightId = flightsRS.getInt(1);

                //grab the Longitudinal Acceleration double_series for that flight
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < timeSeries.size(); i++) {
                        ...
                        if (current < minValue || current > maxValue) {
                            System.err.println("Longitudinal Acceleration exceedence at entry: " + i);
