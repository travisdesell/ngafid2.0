    ...
    public void writeFlightData(StringBuffer buffer, Map<String, Object> scopes){
        ...
        for (int i = 0; i < length; i++) {
            ...
            && !Double.isNaN(parameters.get(LATITUDE).get(i))){
                ...
                buffer.append("DATA, ").append(i).append(",").append(NULL_DATA).append(parameters.get(LONGITUDE).get(i)).append(",").append(parameters.get(LATITUDE).get(i)).append(",").append(parameters.get(ALT).get(i)).append(",").append(getZeros(4)).append(parameters.get(PITCH).get(i)).append(",").append(parameters.get(ROLL).get(i)).append(",").append(parameters.get(HEADING).get(i)).append(",").append(parameters.get(IAS).get(i)).append(getZeros(55)).append(Double.isNaN(rpmVal) ? "0" : rpmVal).append(getZeros(7)).append(Double.isNaN(e1EGT) ? "0" : e1EGT).append(getZeros(2)).append("\n");
            }
        }
    }
}
