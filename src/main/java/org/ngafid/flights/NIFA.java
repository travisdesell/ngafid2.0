...
public class NIFA implements Serializable {
    ...
    static {
        ...
        try (Connection connection = Database.getConnection()) {
            ...
            e.printStackTrace();
            System.out.println(
                    "Failed to initialized NIFA event definitions. This wont be a problem unless you plan on using thme");
        }
    }
    ...
    public NIFA(Connection connection, Flight flight) throws SQLException {
        ...
        Object[] a = { latTimeSeries, lonTimeSeries, altTimeSeries, altMSLTimeSeries, timeSeries, dateSeries,
                velocityTimeSeries };
        ...
        if (!Stream.of(latTimeSeries, lonTimeSeries, altTimeSeries, altMSLTimeSeries, timeSeries, dateSeries,
                velocityTimeSeries).anyMatch(Objects::isNull)) {
    ...
    // Creates event of a given type between the given index range and inserts it
    // into the database.
    void createEvent(Connection connection, int eventDefinitionId, int startIndex, int endIndex) {
        ...
        Event e = new Event(0, fleetId, flightId, eventDefinitionId, startIndex, endIndex, startTimestamp, endTimestamp,
                1.0, null);
    ...
    // spooky equation taken from wikipedia -
    ...
    private void process(Connection connection) throws SQLException {
        ...
        for (Itinerary it : itineraries) {
            ...
            for (int i = from + 1; i < until; i++) {
                ...
                // 0-180 degrees, difference between runway and aircraft bearing, used to find
                // flight phase
                ...
                if (bDiff < 20 && !isFirstTurn) {
                    ...
                    if (!isUpwind) {
                        ...
                        // slideshow also mentions checking this anytime after aircraft climbs to 400
                        // feet,
                    ...
                    // if not, flip arbitrary boolean flag for as long as aircraft is outside
                    // corridor,
             ...
             * 
             * double trailingBearing = bearing(runway.lat1, runway.lon1, runway.lat2,
             * runway.lon2);
             * double runwayBearing = bearing(runway.lat1, runway.lon1, runway.lat2,
             * runway.lon2);
             ...
             * double normDeg = (trailingBearing - bearing(lat[i-1], lon[i-1], lat[i],
             * lon[i])) % 360;
             * double bDiff = Math.min(360-normDeg, normDeg);
             * 
             * // normDeg = (runwayBearing - bearing(lat[i-1], lon[i-1], lat[i], lon[i])) %
             * 360;
             ...
             * 
             * // path DFA:
             * // upTrack -> turnCross1 -> crossTrack1 -> turnDown -> downTrack ->
             * turnCross2 -> crossTrack2
