    ...
    public Object handle(Request request, Response response) {
        ...
        try {
            ...
            if (upload == null) {
                ...
                return gson.toJson(new ErrorResponse("File Upload Failure", "A system error occured where this upload was not in the database. Please try again."));
            }
            ...
            if (upload.completed()) {
                ...
                if (!upload.checkSize()) {
                    ...
                    return gson.toJson(new ErrorResponse("File Upload Failure", "An error occurred while putting the chunk files together to make the full uploaded file. The new full file had a different number of bytes than the one that was originally uploaded, so some corruption may have occurred on transfer. Please delete this file, reload the webpage and retry."));
                }
                ...
                if (!newMd5Hash.equals(upload.getMd5Hash())) {
                    LOG.severe("ERROR! Final file had incorrect bytes, original MD5 hash and uploaded MD5 hashes do not match, some data may have been corrupted.");
                    return gson.toJson(new ErrorResponse("File Upload Failure", "An error occurred while putting the chunk files together to make the full uploaded file. The new full file had different bytes than the one that was originally uploaded, so some corruption may have occurred on transfer. Please delete this file, reload the webpage and retry."));
                }
