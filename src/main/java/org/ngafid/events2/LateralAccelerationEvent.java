...
public class LateralAccelerationEvent {

    public static void calculateEvents(Connection connection) {
        try {
            String columnName = "LatAc";
            double minValue = -1.7;
            double maxValue = 4.4;
            ...
            while (flightsRS.next()) {
                //get the ID from each flight that had a Lateral  Acceleration event
                int flightId = flightsRS.getInt(1);

                //grab the Lateral Acceleration double_series for that flight
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < timeSeries.size(); i++) {
                        ...
                        if (current < minValue || current > maxValue) {
                            System.err.println("LateralAcceleration exceedence at entry: " + i);
