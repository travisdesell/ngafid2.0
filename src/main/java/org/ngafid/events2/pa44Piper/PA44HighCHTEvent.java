...
public class PA44HighCHTEvent {

    public static void calculateEvents(Connection connection) {
        try {
            String columnName = "PA44HighCHT";
            double minValue = -30.0;
            double maxValue = 30.0;

            //grab the flight IDs that can have a PA44HighCHT event
            ...
            while (flightsRS.next()) {
                //get the ID from each flight that had a PA44HighCHT event
                int flightId = flightsRS.getInt(1);

                //grab the PA44HighCHT double_series for that flight
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < timeSeries.size(); i++) {
                        ...
                        if (current < minValue || current > maxValue) {
                            System.err.println("PA44HighCHT exceedence at entry: " + i);
                        }
                    }
                }
