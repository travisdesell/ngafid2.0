...
public class PA28LowFuelEvent {

    public static void calculateEvents(Connection connection) {
        try {
            String columnName = "PA28LowFuel";
            double minValue = -30.0;
            double maxValue = 30.0;

            //grab the flight IDs that can have a PA28LowFuel event
            ...
            while (flightsRS.next()) {
                //get the ID from each flight that had a PA28LowFuel event
                int flightId = flightsRS.getInt(1);

                //grab the PA28LowFuel double_series for that flight
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < timeSeries.size(); i++) {
                        ...
                        if (current < minValue || current > maxValue) {
                            System.err.println("PA28LowFuel exceedence at entry: " + i);
