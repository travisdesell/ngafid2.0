package org.ngafid.events2.pa28Piper;
...
public class PA28LowOilPressureEvent {

    public static void calculateEvents(Connection connection) {
        try {
            String columnName = "PA28LowOilPressure";
            double minValue = -30.0;
            double maxValue = 30.0;

            //grab the flight IDs that can have a PA28LowOilPressure event
            ...
            while (flightsRS.next()) {
                //get the ID from each flight that had a PA28LowOilPressure event
                int flightId = flightsRS.getInt(1);

                //grab the PA28LowOilPressure double_series for that flight
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < timeSeries.size(); i++) {
                        ...
                        if (current < minValue || current > maxValue) {
                            System.err.println("PA28LowOilPressure exceedence at entry: " + i);
