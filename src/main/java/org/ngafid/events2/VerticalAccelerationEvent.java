...
public class VerticalAccelerationEvent {

    public static void calculateEvents(Connection connection) {
        try {
            String columnName = "NormAc";
            double minValue = -1.76;
            double maxValue = 4.4;

            //grab the flight IDs that can have a Vertical Acceleration event
            ...
            while (flightsRS.next()) {
                //get the ID from each flight that had a Vertical Acceleration event
                int flightId = flightsRS.getInt(1);

                //grab the Vertical Acceleration double_series for that flight
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < timeSeries.size(); i++) {
                        ...
                        if (current < minValue || current > maxValue) {
                            System.err.println("Vertical Acceleration exceedence at entry: " + i);
