...
public class C182LowFuelEvent {
    ...
    public static void calculateEvents(Connection connection) {
        try {
            ...
            //grab the flight IDs that can have a C182LowFuel event
            ...
            while (flightsRS.next()) {
                //get the ID from each flight that had a C182LowFuel event
                int flightId = flightsRS.getInt(1);

                //grab the C182LowFuel double_series for that flight
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < timeSeries.size(); i++) {
                        ...
                        if (current < minValue || current > maxValue) {
                            System.err.println("C182LowFuel exceedence at entry: " + i);
