...
public class C172HighAltitudeEvent {

    public static void calculateEvents(Connection connection) {
        try {
            String columnName = "AltMSL";
           // double minValue = -30.0;
            double maxValue = 12800.0;
           //double limitValue = 12800.0;

            //grab the flight IDs that can have a C172 High Altitude event
           ...
           // flightsPS.setDouble(2, limitValue);
            System.out.println(flightsPS);

            ResultSet flightsRS = flightsPS.executeQuery();
            while (flightsRS.next()) {
                //get the ID from each flight that had a C172 High Altitude event
                int flightId = flightsRS.getInt(1);

                //grab the C172 High Altitude double_series for that flight
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < timeSeries.size(); i++) {
                        ...
                        if ( current > maxValue) {
                            System.err.println("C172 High Altitude exceedence at entry: " + i);
                        }
                    }
                }
