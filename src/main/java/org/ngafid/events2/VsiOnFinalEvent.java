...
public class VsiOnFinalEvent {

    public static void calculateEvents(Connection connection) {
        try {
            String columnName = "VSpd";

            // the correct value is min=-1000 or max=1000
            double minValue = -1900.0;
            double maxValue = 1900.0;

            //grab the flight IDs that can have a Vsi On Final event
            ...
            while (flightsRS.next()) {
                //get the ID from each flight that had a Vsi On Final event
                int flightId = flightsRS.getInt(1);

                //grab the Vsi On Final double_series for that flight
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < timeSeries.size(); i++) {
                        ...
                        if (current < minValue || current > maxValue) {
                            System.err.println("Vsi On Final exceedence at entry: " + i);
