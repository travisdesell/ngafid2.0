    ...
    try {
      ...
      for (Event event : eventList) {
        ...
        if (event.getStartTime() != null) {
          EventStatistics.updateEventStatistics(connection, fleetId, airframeNameId, adjacencyEventDefinitionId,
      ...
      for (Map.Entry<Integer, EventStats> entry : flightEventStats.entrySet()) {
        ...
        if (stats.eventCount > 0) {
          ...
          EventStatistics.updateFlightsWithoutEvent(connection, fleetId, airframeNameId, adjacencyEventDefinitionId,
              flight.getStartDateTime());
