                ...
                <div>
                    {
                        flights.map((flightInfo, index) => {
                            if(flightInfo != null) {
                                return (
                                    <Flight showPlot={() => {this.props.showPlot();}} showMap={() => {this.props.showMap();}} showCesiumPage={(flightId)=>{this.props.showCesiumPage(flightId);}} flightInfo={flightInfo} navBar={this.props.navBar} pageIndex={index} addCesiumFlightPhase={(phase, flightId) => {this.props.addCesiumFlightPhase(phase, flightId);}} addCesiumEventEntity={(event, flightId) => {this.props.addCesiumEventEntity(event, flightId);}} removeCesiumEntity={(flightId)=>{this.props.removeCesiumEntity(flightId)}}
